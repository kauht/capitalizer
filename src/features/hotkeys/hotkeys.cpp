...
namespace feature {
    ...
    static UINT g_upperVk = VK_PRIOR, g_upperMods = 0;
    static UINT g_lowerVk = VK_NEXT, g_lowerMods = 0;
    static bool g_registered = false;

    static void SaveHotkeys() {
        HKEY key;
        if (RegCreateKeyExW(HKEY_CURRENT_USER, kSettingsKey, 0, nullptr, 0, KEY_WRITE, nullptr, &key, nullptr) != ERROR_SUCCESS) return;
    ...
    void LoadHotkeys() {
        ...
        auto read = [&](const wchar_t* name, UINT& out) {
            ...
            if (RegQueryValueExW(key, name, nullptr, &type, reinterpret_cast<BYTE*>(&d), &size) == ERROR_SUCCESS && type == REG_DWORD) out = d;
        };
    ...
    bool ApplyHotkeys(HWND owner, UINT uVk, UINT uMods, UINT lVk, UINT lMods) {
        ...
        MessageBoxW(nullptr, L"That shortcut is already in use, or both hotkeys are the same. Pick another.", app::kName, MB_OK | MB_ICONWARNING | MB_TOPMOST);
        return false;
    }
