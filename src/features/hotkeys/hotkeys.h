...
namespace feature {
    ...
    // Suspended while the settings window is open so its key picker receives the raw
    // keypress instead of the hotkey firing a transform.
    void UnregisterHotkeys(HWND owner);
    bool HotkeysRegistered();

    // Validate and persist the hotkeys chosen in settings. Registers them to check for
    // conflicts, then immediately unregisters (they stay suspended while the window is
    // open, and are re-registered on close). Warns and returns false on conflict.
