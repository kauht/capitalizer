...
namespace feature {
    ...
    static std::wstring Transform(const std::wstring& s, app::Mode mode) {
        return mode == app::Mode::Upper ? util::ToUpper(s) : util::ToLower(s);
    }

    // Fast path: read/replace the selection with Win32 edit-control messages. Returns
    // true only if a real selection was found and replaced (native Edit / RichEdit).
    // Non-edit windows (browser/Electron surfaces) report no selection, so this returns
    // false and the caller falls back to the clipboard path.
    static bool TryMessageTransform(HWND ctl, app::Mode mode) {
    ...
    // Fallback for apps with no Win32 edit control (browsers, Electron, UWP): copy the
    // selection, transform it, paste it back, then restore the clipboard.
    static void ClipboardTransform(HWND owner, app::Mode mode) {
    ...
    void DoTransform(HWND clipboardOwner, app::Mode mode) {
        ...
        if (ctl && !TryMessageTransform(ctl, mode) && !util::IsEditControl(ctl))
            ClipboardTransform(clipboardOwner, mode);   // non-edit controls only (browser/Electron/UWP)
