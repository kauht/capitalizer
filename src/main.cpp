...
// Auto-save the hotkeys chosen in the settings window. The window keeps the global
// hotkeys suspended so its key picker still receives raw keypresses, so here we only
// *validate* by registering then immediately unregistering; the committed values are
// (re)registered for real when the window closes (WM_DESTROY). On conflict, warn and
...
.titlebar { flex: none; height: 46px; display: flex; align-items: center;
...
.hint { flex: 1; font-size: 11px; color: #74747e; }
.saved { display: inline-flex; align-items: center; gap: 6px; font-size: 11px;
  color: #6c6c76; transition: color .25s; }
.saved svg { width: 13px; height: 13px; display: block; }
.saved.pulse { color: #5ac37a; }
</style></head><body>
...
<div class="statusbar">
  ...
  <div class="saved" id="saved"><svg viewBox="0 0 14 14" fill="none"><path d="M2.5 7.5 L5.5 10.5 L11.5 4" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>Saved</div>
</div>
<script>
...
vp.addEventListener('message', function(e){
  ...
  if(p[0]==='init'){
    ...
    clearTimeout(s._t);
    s._t=setTimeout(function(){ s.classList.remove('pulse'); }, 1100);
  }
});
render();
    ...
    if (wcsncmp(msg, L"apply|", 6) == 0) {
        ...
        if (swscanf_s(msg + 6, L"%u|%u|%u|%u|%d", &uv, &um, &lv, &lm, &as) == 5) {
            if (ApplyHotkeys(uv, um, lv, lm)) {
                SetAutostart(as != 0);
                if (g_webView) g_webView->PostWebMessageAsString(L"saved");
            } else if (g_webView) {
