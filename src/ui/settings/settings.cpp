...
namespace ui {
    ...
    static void ApplyAcrylic(HWND hwnd) {
        ...
        auto setca = user32 ? reinterpret_cast<SetWindowCompositionAttributeFn>(GetProcAddress(user32, "SetWindowCompositionAttribute")) : nullptr;
        if (setca) {
    ...
    static void SendInitToWeb() {
        if (!g_webView) return;
        const feature::HotkeyState u = feature::UpperHotkey();
        const feature::HotkeyState l = feature::LowerHotkey();
        wchar_t buf[128];
        swprintf_s(buf, L"init|%u|%u|%u|%u|%d", u.vk, u.mods, l.vk, l.mods, feature::IsAutostartEnabled() ? 1 : 0);
        g_webView->PostWebMessageAsString(buf);
    }

    static void OnWebMessage(LPCWSTR msg) {
    ...
    static void CreateWebView() {
        ...
        if (GetEnvironmentVariableW(L"LOCALAPPDATA", local, MAX_PATH)) udf = std::wstring(local) + L"\\Capitalizer";
                        ...
                        Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                            [](HRESULT, ICoreWebView2Controller* controller) -> HRESULT {
                                ...
                                if (settings) {
                                    ...
                                    if (SUCCEEDED(settings.As(&s3))) s3->put_AreBrowserAcceleratorKeysEnabled(FALSE);
                                }
    ...
    void ShowSettings(HWND owner) {
        ...
        g_settingsHwnd = CreateWindowExW(WS_EX_APPWINDOW, kSettingsClass, app::kName, style, x, y, w, h, nullptr, nullptr, inst, nullptr);
        if (!g_settingsHwnd) return;

        HICON iconBig = LoadIconW(inst, MAKEINTRESOURCEW(1));
        HICON iconSmall = static_cast<HICON>(LoadImageW(inst, MAKEINTRESOURCEW(1), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR));
        SendMessageW(g_settingsHwnd, WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(iconBig));
        SendMessageW(g_settingsHwnd, WM_SETICON, ICON_SMALL, reinterpret_cast<LPARAM>(iconSmall));
