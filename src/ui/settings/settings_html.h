...
inline constexpr wchar_t kSettingsHtml[] = LR"HTML(<!doctype html>
<html>
<head>
<meta charset="utf-8">
<style>
  ...
  * {
  ...
  .hk.capturing {
    border-color: #4c9bf5;
    background: rgba(76, 155, 245, 0.14);
    color: #bcd8fb;
  }
  ...
  <div class="panel">
    <div class="rows">
      <div class="row">
        ...
        <div class="hk" id="hkU" tabindex="0">Unset</div>
      </div>
      <div class="row">
        <div class="label">lowercase</div>
        <div class="hk" id="hkL" tabindex="0">Unset</div>
      </div>
      <div class="row" style="border-bottom: none">
        <div class="label">Start with Windows</div>
        <div class="toggle" id="tg" tabindex="0">
          <div class="knob"></div>
        </div>
      </div>
    </div>
...
<script>
  ...
  function keyName(vk) {
    var m = {
      8: 'Backspace', 9: 'Tab', 13: 'Enter', 20: 'Caps Lock', 27: 'Esc', 32: 'Space',
  ...
  function render() {
    var u = document.getElementById('hkU');
    var l = document.getElementById('hkL');
  ...
  function autoSave() {
    vp.postMessage('apply|' + st.hkU.vk + '|' + st.hkU.mods + '|' + st.hkL.vk + '|' + st.hkL.mods + '|' + (st.autostart ? 1 : 0));
  }
